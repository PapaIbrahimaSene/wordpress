...
jQuery(document).on('click', '#insert_ads_vi_signup2', function(){
	...
	if(location.search == '?post_type=insertpostsignature&page=post-signature'){
		jQuery('#insert_ads_vi_signup').trigger('click');
	} else {
		location.href = '/wp-admin/edit.php?post_type=insertpostsignature&page=post-signature';
	}
})
				...
				((jQuery('#insert_ads_vi_code_settings_keywords').val() == '') || ((jQuery(jQuery('#insert_ads_vi_code_settings_keywords').val().match(/./g)).not(jQuery('#insert_ads_vi_code_settings_keywords').val().match(keywordsRegex)).get().length == 0) && (jQuery('#insert_ads_vi_code_settings_keywords').val().length < 200)))
					...
					jQuery.post(
						jQuery('#insert_ads_admin_ajax').val(), {
							...
							if(response.indexOf('###SUCCESS###') !== -1) {
								jQuery('.ui-dialog-titlebar').find('button').last().button('enable').click();
								var successText = '<div id="sucessText" class="notice notice-success is-dismissible post-signature-notice-welcome"><p>Settings saved!</p></div>';
...
jQuery(document).ready(function() {
	jQuery('.post-new-php.post-type-insertpostsignature select[name="ad_position"] option[value="top"]').attr('selected', true);
	jQuery('.post-new-php.post-type-insertpostsignature select[name="ad_position"] option[value=""]').attr('selected', false);
	jQuery('.post-new-php.post-type-insertpostsignature #paragraph_number').hide();

	jQuery('.post-type-insertpostsignature #search-submit').val('Search').addClass('button-primary');
	jQuery('.post-type-insertpostsignature #post-search-input').attr('placeholder', 'Enter advert title');

	jQuery('.post-type-insertpostsignature #bulk-action-selector-top').before('<p class="bulk">Bulk actions:</p>');
	jQuery('.post-type-insertpostsignature #bulk-action-selector-top option[value="-1"]').text('Select action');

	var getTitle = jQuery('.post-new-php.post-type-insertpostsignature #title').remove();
	jQuery('#ipa_meta .inside').prepend('<p id="place-title"><label>Signature Title:</label></p>');
	jQuery(getTitle).appendTo('#place-title');

    var publishBtn = jQuery('body.post-new-php.post-type-insertpostsignature #publish').remove();
	...
	jQuery('#set-submit').click(function(e){
		...
		jQuery.ajax({
			url : 'edit.php?post_type=insertpostsignature&page=post-signature',
			method : 'POST',
			data : dataForm,
			success : function(){
				var sucdial = jQuery('<div id="sucdial">Succesfully updated!</div>').hide().fadeIn('1000');
