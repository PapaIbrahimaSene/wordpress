...
* Description: Allows you to insert Content after paragraphs of your post
*/

/*  Copyright 2019 Papa Ibrahima Sene
*/

/**
* Post Signature Class
*/
class InsertPostSignature {
	/**
	* Constructor
	*/
	public function __construct() {
        ...
        $this->plugin->name         = 'post-signature'; // Plugin Folder
        $this->plugin->displayName  = 'Post Signature'; // Plugin Name
        $this->plugin->posttype 	= 'insertpostsignature';
        $this->plugin->version      = '1.01';
        $this->plugin->folder       = plugin_dir_path( __FILE__ );
        $this->plugin->url          = plugin_dir_url( __FILE__ );
        $this->plugin->content_screen_key = $this->plugin->name . '-ads-display-chosen-once';
    ...
    */
    function adminPanelsAndMetaBoxes() {
		...
		add_meta_box( 'ipa_meta', __( 'New Signature', $this->plugin->name ), array( &$this, 'displayMetaBox' ), $this->plugin->posttype, 'normal', 'high' );
			...
			$this->errorMessage = __( 'Something went wrong. Please try to save again.', $this->plugin->name );
				...
				if ( isset( $_REQUEST[$this->plugin->name] ) ) {
					...
					if( ( count( $_REQUEST[$this->plugin->name] ) == 1 && !isset( $_REQUEST[$this->plugin->name]['css'] ) ) || count( $_REQUEST[$this->plugin->name] ) > 1  ) {
						update_option( $this->plugin->content_screen_key, 1 );
					}
					// sanitise the array
				...
				$this->message = __( 'Post Signature Settings Saved.', $this->plugin->name );
			}
		}
	...
	*/
	function displayMetaBox( $post ) {
		...
		?>
		<p>
			<label for="ad_code">Signature code:</label>
		...
		<p>
			<label for="ad_position"><?php _e( 'Display the signature:', $this->plugin->name ); ?></label>
		...
		<p><input readonly name="ad_unit" id="ad_unit" type="text" value="<?php echo $adUnit ? $adUnit : 'Signature post'; ?>"></p>
		<?php
	}
	...
	*/
	function displayOptionsMetaBox( $post ) {
		...
		?>
		<p>
			<label for="ipa_disable_ads"><?php _e( 'Disable Signatures', $this->plugin->name ); ?></label>
			<input type="checkbox" name="ipa_disable_ads" id="ipa_disable_ads" value="1"<?php echo ( $disable ? ' checked' : '' ); ?> />
		</p>
		<p class="description">
			<?php _e( 'Check this option if you wish to disable all Post Signatures from displaying on this content.', $this->plugin->name ); ?>
		</p>
		<?php
	}
	...
	*/
	function changeTitlePlaceholder( $title ) {
		...
		if ( $post->post_type == $this->plugin->posttype ) {
			$title = __( 'Signature Title', $this->plugin->name );
		}
	...
	*/
	function changeUpdatedMessages( $messages ) {
		$published_msg = __( 'Signature published.', $this->plugin->name );
		$updated_msg = __( 'Signature updated.', $this->plugin->name );
		// change the messages for first time user, if where to display ads options are not set
		if ( !get_option( $this->plugin->content_screen_key ) ) {
			$published_msg = sprintf( __( 'Signature published. Now, go to the <a href="%s">settings page</a> to select where you want to display your ads.', $this->plugin->name ), admin_url( 'edit.php?post_type=' . $this->plugin->posttype . '&page=' . $this->plugin->name ) );
			$updated_msg = sprintf( __( 'Signature updated. Now, go to the <a href="%s">settings page</a> to select where you want to display your ads.', $this->plugin->name ), admin_url( 'edit.php?post_type=' . $this->plugin->posttype . '&page=' . $this->plugin->name ) );
		}
...
$insertPostAds = new InsertPostSignature();
...
if(!defined('WP_INSERTCONTENT_URL'))
	define('WP_INSERTCONTENT_URL', plugin_dir_url(__FILE__));
...
function insert_ads_admin_notices() {
		...
		if(!get_user_meta($userId, 'insert_ads_'.WP_INSADS_VERSION.'_admin_notice_dismissed', true)) {
			echo '<div class="notice notice-success insert_ads_notice is-dismissible" style="padding: 15px; background-color: #e81c3a;">';
				echo '<img style="float: left; margin-left: 20px; margin-top: 13px; margin-bottom: 13px; width: 12%;" src="'.WP_INSERTCONTENT_URL.'images/autodeal_post_signature.svg?'.WP_INSADS_VERSION.'" />';
...
function insert_ads_admin_notices_settings() {
		...
		if ($getUrl == '/wp-admin/edit.php?post_type=insertpostsignature'){
		?>
		<div class="notice ins-notice">
		<div id="poststuff">
		<form action="edit.php?post_type=insertpostsignature&page=post-signature" method="post" class="ins-settings">
		                <div class="postbox">
		                    <h3 class="hndle"><?php _e( 'Display Settings', 'post-signature'); ?></h3>
		                    	...
		                    	<p>
									...
									if ( $postTypes ) {
										foreach ( $postTypes as $postType ) {
											...
											<input type="checkbox" name="post-signature[<?php echo $postType->name; ?>]" value="1" id="<?php echo $postType->name; ?>" <?php echo ( isset( get_option( 'post-signature' )[$postType->name] ) ? ' checked' : '' ); ?>/>
											<?php
										}
									}
									?>
									<label for="css"><?php _e( 'Exclude CSS', 'post-signature' ) ;?></label>
									<input type="checkbox" name="post-signature[css]" value="1" id="css"/>
								</p>

								<p class="description">
									<?php _e( 'By default, Post Signatures are wrapped in a container that has some CSS to aid layout. Developers may wish to use their own CSS, and should check this Exclude CSS option.', 'post-signature' ); ?>
								</p>
								<p>
									<input id="set-submit" name="submit" type="submit" name="Submit" class="button button-primary" value="<?php _e( 'Save Settings', 'post-signature' ); ?>" />
								</p>
		                    </div>
		                </div>
		                <!-- /postbox -->
		                ...
		                <!-- /postbox -->
		                <input type="hidden" name="_nonce" value= "<?php echo wp_create_nonce( 'post-signature' . '-nonce' ); ?>" />
	                </form>
					</div>
					</div>
		<?php
